...
function showLongTimeAlert(time) {
  if (time > 45) {
    return `
        <div class="alert alert-danger" role="alert">
          <div class="d-flex justify-content-start align-items-center">
            <i class="fas fa-triangle-exclamation text-danger fs-4 me-3"></i>
            <div>
              <p class="fw-semibold mb-1">Extended Preparation Time</p>
              <p class="small mb-0">This recipe requires more than 45 minutes to prepare. Plan accordingly!</p>
            </div>
          </div>
        </div>
    `;
...
function generateRandomRecipe() {
  ...
  for (let i = 0; i < items[randomIndex].ingredients.length; i++) {
    ...
    <li class="d-flex align-items-start mb-3">
      <div class="ingredient-number me-3">${i + 1}</div>
      <span>${items[randomIndex].ingredients[i]}</span>
    </li>
  `;
  }

  var instructionsHTML = '';
  for (var i = 0; i < items[randomIndex].instructions.length; i++) {
    instructionsHTML += `
     <div class="d-flex align-items-start mb-3">
        <div class="instruction-number me-3">${i + 1}</div>
          <p class="mb-0">${items[randomIndex].instructions[i]}</p>
      </div>
  `;
  }

  var tipCardHTML = '';
  for (var i = 0; i < items[randomIndex].tips.length; i++) {
    tipCardHTML += `
      <div class="tip-card mb-2">
        <div class="d-flex align-items-start">
          <i class="fas fa-circle-check text-warning fs-5 me-3 mt-1"></i>
          <p class="mb-0">${items[randomIndex].tips[i]}</p>
        </div>
      </div>
  `;
  }
  ...
  document.getElementById('main-section').innerHTML = `
   <section class="bg-white recipe-card shadow-lg mb-4">
      <div class="row g-0 ">
        <div class="col-lg-5 recipe-image-container">
          <img src="${items[randomIndex].imgCover}"
            alt="${items[randomIndex].image}" class="recipe-image">

          <div class="rating-badge">
            <div class="d-flex align-items-center gap-2">
              <i class="fas fa-star text-warning"></i>
              <span class="fw-semibold">${
                items[randomIndex].ratingsAverage
              }</span>
              <span class="text-muted small">(${
                items[randomIndex].ratingsQuantity
              } reviews)</span>
            </div>
          </div>
          ...
          <div class="stats-overlay">
            <div class="row g-3 text-center">
              <div class="col-4 d-flex flex-column justify-content-center align-items-center">
                <i class="fas fa-clock text-warning stat-icon d-block"></i>
                <p class="text-muted small mb-1">Prep Time</p>
                <p class="fw-bold mb-0">${items[randomIndex].prepTime} min</p>
              </div>
              <div class="col-4 d-flex flex-column justify-content-center align-items-center">
                <i class="fas fa-fire-burner text-danger stat-icon d-block"></i>
                <p class="text-muted small mb-1">Cook Time</p>
                <p class="fw-bold mb-0">${items[randomIndex].cookTime} min</p>
              </div>
              <div class="col-4 d-flex flex-column justify-content-center align-items-center">
                <i class="fas fa-users text-primary stat-icon d-block"></i>
                <p class="text-muted small mb-1">Servings</p>
                <p class="fw-bold mb-0">${items[randomIndex].servings}</p>
              </div>
            </div>
          </div>
        </div>

        <div class="col-lg-7 p-4 p-lg-5 content-area">
          <div class="d-flex flex-column flex-md-row gap-3 align-items-start justify-content-between mb-4">
            <div>
              <div class="d-flex gap-2 mb-3">
                <span class="badge bg-success badge-pill">${
                  items[randomIndex].difficulty
                }</span>
                <span class="badge bg-primary badge-pill">${
                  items[randomIndex].category
                }</span>
              </div>
              <h3 class="display-6 fw-bold mb-2">${items[randomIndex].name}</h3>
            ...
            <div class="d-flex gap-2 mx-auto mx-lg-0">
              ...
              <button onclick="generateRandomRecipe()" class="btn-action">
                <i class="fa-solid fa-shuffle"></i>
              </button>
            </div>
          </div>

          ${showLongTimeAlert(Number(items[randomIndex].totalTime))}

          <ul class="nav nav-tabs mb-4" id="recipeTabs" role="tablist">
            <li class="nav-item flex-fill" role="presentation">
              <button class="nav-link active w-100" id="ingredients-tab" data-bs-toggle="tab"
                data-bs-target="#ingredients" type="button" role="tab" aria-controls="ingredients" aria-selected="true">
                <i class="fas fa-list-check me-1"></i>
                <span class="d-none d-md-inline">Ingredients</span>
              </button>
            </li>
            <li class="nav-item flex-fill" role="presentation">
              <button class="nav-link w-100" id="instructions-tab" data-bs-toggle="tab" data-bs-target="#instructions"
                type="button" role="tab" aria-controls="instructions" aria-selected="false">
                <i class="fas fa-book-open me-1"></i>
                <span class="d-none d-md-inline">Instructions</span>
              </button>
            </li>
            <li class="nav-item flex-fill" role="presentation">
              <button class="nav-link w-100" id="nutrition-tab" data-bs-toggle="tab" data-bs-target="#nutrition"
                type="button" role="tab" aria-controls="nutrition" aria-selected="false">
                <i class="fas fa-chart-pie me-1"></i>
                <span class="d-none d-md-inline">Nutrition</span>
              </button>
            </li>
            <li class="nav-item flex-fill" role="presentation">
              <button class="nav-link w-100" id="tips-tab" data-bs-toggle="tab" data-bs-target="#tips" type="button"
                role="tab" aria-controls="tips" aria-selected="false">
                <i class="fas fa-lightbulb me-1"></i>
                <span class="d-none d-md-inline">Tips</span>
              </button>
            </li>
          </ul>

          <div class="tab-content" id="recipeTabContent">
            <div class="tab-pane fade show active h-100" id="ingredients" role="tabpanel"
            ...
            <div class="tab-pane fade h-100" id="instructions" role="tabpanel" aria-labelledby="instructions-tab"
              tabindex="0">
              ${instructionsHTML}
            </div>

            <div class="tab-pane fade h-100" id="nutrition" role="tabpanel" aria-labelledby="nutrition-tab"
              tabindex="0">
              <div class="row g-3">
                <div class="col-md-6">
                  <div class="nutrition-card d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center">
                      <div class="nutrition-icon-box bg-warning bg-opacity-25 me-3">
                        <i class="fas fa-fire text-warning"></i>
                      </div>
                      <span>Calories</span>
                    </div>
                    <span class="fw-bold fs-5">${
                      items[randomIndex].nutrition.calories
                    }</span>
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="nutrition-card d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center">
                      <div class="nutrition-icon-box bg-primary bg-opacity-25 me-3">
                        <i class="fas fa-dumbbell text-primary"></i>
                      </div>
                      <span>Protein</span>
                    </div>
                    <span class="fw-bold fs-5">${
                      items[randomIndex].nutrition.protein
                    }</span>
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="nutrition-card d-flex align-items-center justify-content-between">
                    ...
                    <span class="fw-bold fs-5">${
                      items[randomIndex].nutrition.carbs
                    }</span>
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="nutrition-card d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center">
                      <div class="nutrition-icon-box bg-danger bg-opacity-25 me-3">
                        <i class="fas fa-droplet text-danger"></i>
                      </div>
                      <span>Fat</span>
                    </div>
                    <span class="fw-bold fs-5">${
                      items[randomIndex].nutrition.fat
                    }</span>
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="nutrition-card d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center">
                      <div class="nutrition-icon-box bg-success bg-opacity-25 me-3">
                        <i class="fas fa-seedling text-success"></i>
                      </div>
                      <span>Fiber</span>
                    </div>
                    <span class="fw-bold fs-5">${
                      items[randomIndex].nutrition.fiber
                    }</span>
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="nutrition-card d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center">
                      <div class="nutrition-icon-box bg-info bg-opacity-25 me-3">
                        <i class="fas fa-cube text-info"></i>
                      </div>
                      <span>Sodium</span>
                    </div>
                    <span class="fw-bold fs-5">${
                      items[randomIndex].nutrition.sodium
                    }</span>
                  </div>
                </div>
              </div>
            ...
            <div class="tab-pane fade h-100" id="tips" role="tabpanel" aria-labelledby="tips-tab" tabindex="0">
             ${tipCardHTML}
            </div>
          </div>

          <div class="mt-4 pt-4 border-top">
            <button onclick="generateRandomRecipe()" class="btn btn-danger shadow">
